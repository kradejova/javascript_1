const predpoved = [
  ...
  const pocasiDen = document.querySelector("#weather-cards")

  console.log(pocasiDen)

  const pocasiId = window.location.hash.slice(1);
  const PocasiData = predpoved.find((pocasi) => predpoved.id === pocasiId);
  
  pocasiData.innerHTML += `
  <article>
